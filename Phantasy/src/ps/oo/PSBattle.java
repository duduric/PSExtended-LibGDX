...
public class PSBattle {
	...
	public BattleOutcome startBattle(List<Battler> battlers) {
		...
		BattleOutcome battleResult = battleLoop(battlers);
		...
		battlelog(attacker.getName() + " is attacking " + defenser.getName() + ": ");	
	        ...
	        if(attacker instanceof EnemyBattler) {
	        	...
	        	if(weapon!=null && weapon.type == ItemType.PISTOL) {
	        		Collections.sort(allBattlers, Battler.getNaturalComparator());
	        		for(Battler shooted: allBattlers) {
	        			if(shooted instanceof EnemyBattler && ((EnemyBattler) shooted).hp > 0) {
	        				...
	        				hit(shooted, -weapon.itemstat /2); // Pistols have 20/40/60 modifiers and inflict 10/20/30 damage
	        			}
	        		}
	        		Collections.sort(allBattlers, Battler.getPrecedenceComparator());
	        		return;
	        	}
	...
	public static void setTargets(List<Battler> battlers) {
		...
		for(Battler b: battlers) {
			...
			// Battler is dead, can't attack anyone
			if(b.getHp() <=0) {
				b.target = null;
				continue;
			}
